...
1. **電卓UI**:
   ...
   - 成功時は入力欄と分数表示を更新し、余計な結果テキストを下部に重複表示しない。
   - 分数表示の横線は分母の表示幅に追従させる。
2. **安全な計算ロジック (`app/services/calculator.py`)**:
   ...
   - 非常に小さい非ゼロの浮動小数も `0` に丸め落とさず、分数表示または小数表示できるようにする。
...
4. **エラーハンドリングとロギング**:
   ...
   - ユーザー入力由来の例外とシステムエラーは分けて扱い、`except Exception` で隠蔽しすぎないこと。

### 4. テストと品質保証要件
- ユニットテスト (`test_calculator.py`) でコアロジックと両APIエンドポイントを検証し、**カバレッジを100%** に近づけること。
- Playwright を用いたE2Eテスト (`test_ui.py`) で、ブラウザ上でのボタンクリックと計算結果の表示を検証すること。Playwright が入っていないローカル環境ではテストをスキップできるようにし、CI では Docker または Playwright 公式イメージ経由で実行すること。
...
- **GitHub Actions (`ci.yml`)**:
  ...
  - `update-coverage.yml` を別途用意し、README.md のカバレッジブロックを自動更新できるようにすること。
...
- Ruff (Linter/Formatter): `uv run ruff check app tests`
- Mypy (型チェック): `uv run mypy app`
